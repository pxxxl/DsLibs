...
namespace me {
	...
	public:
		matrix() :line(0), column(0) {
			static_assert(is_decimal<T>(T()) || is_integer<T>(T()), "Please use Interger or Demical type to initialize a matrix");
		};
		...
		matrix(const matrix<T>& sample) :line(sample.line), column(sample.column) {
			static_assert(is_decimal<T>(T()) || is_integer<T>(T()), "Please use Interger or Demical type to initialize a matrix");
			data = new T * [line];
			handle_memory_alloc(data);
			for (unsigned i = 0; i < line; i++) {
		...
		matrix(const sparseMatrix<T>& sample) : line(sample.line) column(sample.column), {
			for (auto& item : sample.data) {
				data[item.line][item.column] = item.value;
			}
		}
			matrix& operator=(const matrix& sample) {
			for (unsigned i = 0; i < line; i++) {
				for (unsigned j = 0; j < column; j++) {
					data[i][j] = sample.data[i][j];
				}
	...
	inline matrix<T> matrix<T>::std_line_form() const
	{
		...
		while (cur_line < line && cur_column < column) {
			...
			//if cursor element is not zero, turn it to 1, and clear all the element in this column
				...
				holder = sample.data[cur_line][cur_column];
				for (unsigned i = cur_column; i < column; i++) {
					sample.data[cur_line][i] /= holder;
				}
